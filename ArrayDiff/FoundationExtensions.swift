...
public extension IndexSet {
	...
	*/
    ...
    let origins = ascending ? AnySequence(self) : AnySequence(reversed())
    origins.forEach { index in
...
public extension Array {
    ...
    NSIndexSet(indexSet: indexes).enumerateRanges(options: []) { nsRange, _ in
    ...
    NSIndexSet(indexSet: indexSet).enumerateRanges(options: .reverse) { nsRange, _ in
	...
	public mutating func insertElements(_ newElements: [Element], atIndexes indexes: IndexSet) {
		...
		var i = 0
    NSIndexSet(indexSet: indexes).enumerateRanges(options: []) { range, _ in
